    ...
    if ~isempty(EEG.epoch)
        for i=1:EEG.trials
            if length(EEG.epoch(i).eventlatency) == 1
                ...
                % Samran 19-Oct-2017 15:55: removed cell2mat
                itemx = EEG.epoch(i).eventitem;
                % EEG.epoch(i).eventflag = {EEG.EVENTLIST(indexEL).eventinfo(itemx).flag};
                % Samran 19-Oct-2017 15:58: removed {} to save double value
                % instead of cell datatype value
                EEG.epoch(i).eventflag = EEG.EVENTLIST(indexEL).eventinfo(itemx).flag;
